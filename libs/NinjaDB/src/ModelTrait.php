...
trait ModelTrait {
		...
		if ( is_array( $selects ) ) {
			if ( isset( $this->statements['selects'] ) ) {
				$this->statements['selects']
					= array_merge( $this->statements['selects'], $selects );
			} else {
	...
	public function orWhere(
		...
		if ( is_array( $key ) ) {
			foreach ( $key as $field ) {
				$this->whereHandler( $field[0], $field[1], $field[2], 'OR',
					$isSearch );
			}
	...
	public function whereHandler(
		...
		$this->statements['wheres'][] = compact( 'key', 'operator', 'value',
			'joiner', 'is_search' );
	...
	public function groupBy($field) {
		$this->statements['group_by'] = 'GROUP BY ' . $field;
		return $this;
	}
	...
	public function getSQL( $type = 'select' ) {
		...
		if ( $type == 'select' ) {
			$typeArray = array(
				'SELECT' . ( isset( $this->statements['distinct'] )
					? ' DISTINCT' : '' ),
				isset( $this->statements['selects'] ) ? implode( ', ',
					$this->statements['selects'] ) : '*',
				'FROM'
			);
		} else if ( $type == 'delete' ) {
		...
		$sqlArray = array(
			$this->selected_table,
			$wheres,
			isset( $this->statements['group_by'] )
				? $this->statements['group_by'] : '',
			isset( $this->statements['order_by'] )
				? $this->statements['order_by'] : '',
	...
	public function handleWheres( $wheres, $joinWhere = true ) {
		...
		foreach ( $wheres as $statement ) {
			...
			if ( $statement['is_search'] ) {
				...
				$values = is_array( $statement['value'] ) ? $statement['value']
					: [ $statement['value'] ];

				$criteria .= $statement['joiner'] . ' ' . $statement['key']
				             . ' ' . $statement['operator'] . ' ('
				             . implode( ', ',
				...
				$valuePlaceholder
					              = $this->getValuePlaceholder( $statement['value'] );
				$criteria         .= $statement['joiner'] . ' '
				                     . $statement['key'] . ' '
				                     . $statement['operator'] . ' '
				                     . $valuePlaceholder . ' ';
