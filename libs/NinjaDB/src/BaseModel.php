...
class BaseModel {
	 ...
	 */
	public function get() {
		$result = $this->db->get_results( $this->getSqlStatement(),
			$this->outputType );
	...
	public function update( $data ) {
		if ( isset( $this->statements['wheres'] )
			...
			$result = $this->db->update( $this->selected_table, $data,
				$whereArray, null, $bindings );
			$this->reset();
			if ( false === $result ) {
			...
			return $this->throwError( __( "Where clause does not exist, Please add where clause first",
				'ninjadb' ) );
		}
	}
	...
	public function delete( $indexID = false, $filed = 'id' ) {
		...
		if ( $indexID ) {
			$result = $this->db->delete( $this->selected_table,
				array( $filed => $indexID ) );
			$this->reset();
		...
		if ( isset( $this->statements['wheres'] )
			...
			foreach ( $this->statements['wheres'] as $where ) {
				$whereArray[ $where['key'] ] = $where['value'];
				$bindings[]
				                             = $this->getValuePlaceholder( $where['value'] );
			}
